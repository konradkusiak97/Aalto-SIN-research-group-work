...
# ***** Main informations ******
#
# scan_type: 'didv'='dIdV''='didv-single' -- only dIdV for one voltage = V ; 'v-scan'='V-scan'='Voltage-scan' -- both STM & dIdV scan - V .. Vmax; 'STM'='STM-single' -- STM for one Voltage = V, use V-scan rather #
scan_type = 'v-scan'
# tip_type: 'fixed'='f' -- for stiff/metal tip apexes ; 'relaxed'='r' -- for flexible tip apexes (precalculated by PP-AFM) . For this option you have to have "installed" PPAFM in your PPSTM directory #
tip_type = 'relaxed'
# V: voltage of !!!! V = Vmin for SCANs (Vscan, states) !!!! #
V = -0.5
# V_max: max voltages for SCANs ;  note: V_min >= -2.0 V ; V_max <= 2.0 V (othervise changes in the later code needed) #
V_max = +0.5
# dV: voltage step , dV <= 0.1 V ; for Vscan and STM only#
dV = 0.1
# eta: Lorentzian width of states in energy scale: typically 0.1; can be in range of 0.3-0.05 eV in some cases (low amount of layers ...) even up to 1.0 eV #
eta = 0.1
# WF_decay: Only for STM or Vscan: 0.0 <= WF_decay <= 1.0 ; How fast WorkFunction tunnelling barrier is changing with Voltage : (WF = WF_0 + V*WF_decay) -- 0.0 no change ; 1.0 - the same change as voltage #
WF_decay = 0.5
# tip_orb: 's' ; 'pxy' -- px & py ; 'spxy' -- 50% s & 50% pxy ; '5spxy' -- 5% s & 95% pxy ; '10spxy' -- 10% s & 90% pxy ; 'CO' -- 13% s & 87% pxy (PRL 119, 166001 (2017)) ; 'pz' ; For sample_orbs = 'sp' , possible 'dz2' and 'dxzyz' -- dxz & dyz #
tip_orb = 'pxy'
# sample_orbs: orbitals of the sample 'sp' (light atoms only, faster) or 'spd' (all atoms) #
sample_orbs = 'sp'
# dft_code: 'fireball'='Fireball'='FIREBALL' ; 'aims'='AIMS'='FHI-AIMS' ; 'cp2k'='CP2K' ; 'gpaw'='GPAW' #
dft_code = 'fireball'
# geometry_file: E.G. 'input.xyz' , 'input.bas' , 'geometry.in'; None for GPAW #
geometry_file = 'crazy_mol.xyz'
# pbc: (0,0) = None = False -- only original geometry ; (0.5,0.5) -- 2x2 cell ; (1,1) -- 3x3 cell (around original) ; (2,2) -- 5x5 cell (around original) ... #
pbc = (0, 0)
# lvs: None ; [[ax,ay,0],[bx,by,0]],[0,0,cz]] or [[ax,ay],[bx,by]] ; 'input.lvs' -- files with specified cell ; in FHI-AIMS & GPAW allready specified with geometry #
lvs = None
# spin: None=False ; for FHI-AIMS & CP2K: None -- spin-unpolarized/spin-restricted calc. ;  'both' , 'up'='alpha' or 'down" (last 3 are for spin-polarizes or spin-unrestricted calculations) #
spin = None
# cp2k_name: Name used in CP2K calculations or GPAW calc #
cp2k_name = 'crazy_mol'  
#
...
#
# Q: charge (PP-AFM) ; Ocharge PP-AFM complex_tip autumn 2018) ; [e] (monopole), [e*A] (dipole), [e*A^2] (quadrupole) #
Q = 0.00
# K: x stiffness (PP-AFM master autumn 2018); klat (PP-AFM dev/OpenCl autumn 2018); Oklat (PP-AFM complex_tip autumn 2018) ; [N/m] #
K = 0.50
# data_format: 'xsf'='XSF' ; 'npy'='NPY' ; -- format in which PPpos are stored from PP-AFM run #
data_format = 'npy'
#
# *****Output options ******
#
# PNG: True / False -- plot "png" images (2D constant height) #
PNG = False
# WSXM: True / False -- write ".xyz" WSxM files (2D constant height) #
WSxM = False
...
# plot_atoms: True / False -- plot geometry (position of atoms into the PNG images and into the XSF files). You have to have your geometry, which you want to plot in input_plot.xyz. This doesn't change the name of the output files #
plot_atoms = True
#
# ***** Advanced options ******
#
...
# fermi: None=0.0 -- no change to the Fermi Level ; -0.1 -- shifts the Fermi Level by 0.1 eV lower ... #
fermi = None
# cut_min: cut out all orbitals lower than  -2.5 eV bellow Rermi (should be: cut_min <= Vmin-2*eta) . taken to the Fermi Level #
cut_min = -2.5
# cut_out: cut out all orbitals higher than -2.5 eV above  Fermi (should be: cut_max >= Vmax+2*eta) . taken to the Fermi Level #
cut_max = +2.5
...
import numpy as np
import matplotlib
# Force matplotlib to not use any Xwindows backend. ## !!! important for working on clusters !!!!
matplotlib.use('Agg')
